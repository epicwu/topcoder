...
public:
	vector<int> Girl;
	vector<int> EvilP;
	vector<long long> EvilN;
	bool Check(long long  MAX)
	{
		sort(Girl.begin(),Girl.end());
		for (int i=0;i<EvilP.size();++i)
		for (int j=i+1;j<EvilP.size();++j)
		if (EvilP[i] > EvilP[j])
		{
			swap(EvilP[i],EvilP[j]);
			swap(EvilN[i],EvilN[j]);
		}
		for (int i=0;i<Girl.size();++i)
		{
			int k = int(EvilP.size()) - 1;
			while (k >= 0 && EvilP[k] > Girl[i]) --k;
			long long tmp = MAX;
			while (k>=0 && tmp>0)
			{
				long long tt = min(tmp, EvilN[k]);
				EvilN[k]-=tt;
				tmp-=tt;
				--k;
			}
		}
		for (int i=0;i<EvilN.size();++i)
		if (EvilN[i] > 0) return false;
		return true;
	}
	long long minimalFatigue(vector <int> magicalGirlStrength, vector <int> enemyStrength, vector<long long> enemyCount) {
		long long e_n = 0;
		for (int i=0;i<enemyCount.size();++i) e_n+= enemyCount[i];
		long long l = 0, r = e_n;
		while (l<r)
		{
			long long mid = (l+r)/2;
			...
			EvilP = enemyStrength;
			EvilN = enemyCount;
			if (Check(mid)) r = mid; else
							l = mid + 1;
		}
		if (Check(l)) return l; else return -1;
	}
};
