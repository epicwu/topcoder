...
public:
	...
	int minPresses(vector <string> lines) {
		...
		{
			int tmp = lines[i].size() + 1 +2;
			if (prefix(now,lines[i]))
				tmp = lines[i].size() - now.size() + 1;
			for (int j = 0; j < history.size();++j)
				if (prefix(history[j],lines[i]))
					tmp = min(tmp,int(lines[i].size()) - int(history[j].size()) + 3);
			ans += tmp;
			string tstr = "";
			for (int j = 0; j < lines[i].size();++j)
			{
				tstr += lines[i][j];
				history.push_back(tstr);
			}
			now = lines[i];
		}	
		return ans;
