...
public:
	...
	double getAverage(vector <string> ratings) {
		...
		tmp +=" " + ratings[i];
		cout<<tmp<<endl;
		vector<int> x = Decode(tmp);
		int N = x.size();
		cout<<N<<endl;
		int R = N / 20 + int((N % 20) > 0);
		int ptr = 0;
		int elly = x[0];
		sort(x.begin(),x.end());
		for (int i=0;i<N/2;++i)
		...
		while (ptr<N)
		{
			...
			for (int i=0;i<R;++i)
			{
				if (x[ptr] == elly) flag = true;
				sum += x[ptr];
				ptr ++;
				if (ptr>=N) break;
			}
			if (flag) ans = ans + elly; else
			ans += double(sum)/R;
		}
		int numOfRoom=  (N+R-1)/R;
		if (flag) return ans/double(numOfRoom); else
		{
		 return ans/(double(N)/double(R));
		}
	}
};
...
int main() {
	...
	{
	// ----- test 0 -----
	string t0[] = {"3 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 2 1"};
